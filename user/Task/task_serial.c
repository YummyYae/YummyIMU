...
{
    DL_UART_Main_changeConfig(UART_OUT_INST);
...
static void serial_collect_byte(uint8_t rx)
{
    ...
    } else if (gRxIndex < (UART_RX_LINE_SIZE - 1U)) {
 ...
 */
static void command_handle(RuntimeState_t *state, char *line)
{
    char *p = skip_spaces(line);

    TaskSerial_PauseReport(state);
    command_ack_echo(line);

    if (strncmp(p, "CAL", 3U) == 0) {
        uint32_t wait_s;
        uint32_t record_s;
        p += 3;
        if ((parse_u32_arg(&p, &wait_s) == 0U) ||
            ...
            TaskSerial_Write("ERROR:BAD_ARG\n");
            return;
        }
    ...
    } else if (strncmp(p, "MODE", 4U) == 0) {
        p = skip_spaces(p + 4);
        if (strncmp(p, "DEBUG", 5U) == 0) {
    ...
    } else if (strncmp(p, "STATUS", 6U) == 0) {
        serial_write_status_snapshot(state);
    } else {
        TaskSerial_Write("ERROR:UNKNOWN_COMMAND\n");
    }
}
...
uint8_t TaskSerial_PollCommand(RuntimeState_t *state)
{
    ...
    if (TaskSerial_TakeOverflow() != 0U) {
        TaskSerial_PauseReport(state);
        TaskSerial_Write("ERROR:COMMAND_OVERFLOW\n");
        handled = 1U;
    }
